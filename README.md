...
- Version 2.6.2 stops a busy night being refused. Printing was paused from around 24 Human Remains entries, and compaction was already engaged when it happened — there was one level, and once it had run there was nothing further to try. There are now three, in order of what is cheapest to give up: the first tightens type and leading; the second also reclaims the blank writing rows, close to two inches down the Human column that the first step never touched, and shrinks the notes area; the third is an emergency setting that tightens everything again and releases the spare row from any section that already has entries. Capacity roughly doubles and then doubles again — about 20 entries at the first step, 34 at the second, 50 at the third, with 48 clearing the notes block by a third of an inch. Past that printing is still refused rather than clipping silently. The third step is deliberately uncomfortable to read at 7.8pt and should be rare. **Check a compacted sheet on paper as part of the print-quality gate below; the smallest setting is the one worth judging at arm's length.**
- Version 2.6.1 fixes pinning, which only ever worked from the first blank row of a section: Deliver and FDP show three, so aiming at the bottom row — the natural gesture, and the one described above — hit no drop target and appended the entry unpinned instead. Every blank row now takes the drop. A combined `//` row is counted as the two funeral homes it names rather than as one, so a section of seven rows with two pairs reports nine; card widths can be dragged down to each column's real minimum, 1.62in for Cremated rather than a single hardcoded 2.05in that matched neither column; and the resize grip is fully grabbable, having had half its width clipped out of hit-testing. Up and Down now walk the rows, carrying on past the end of one card into the next section of the same column — on a focused row they move focus, on an open one they commit the line and open the neighbour, which is the difference from Enter's "commit and start the next empty line". Visually the column banners and cards take rounded corners with the banner lifted clear of the card beneath it, every card carries the soft outline that previously marked only the selected one, the masthead moves to the same condensed face as the banners, and the date block leads with the weekday spelled out. **The printed sheet changed in this release; run the physical print-quality gate below before deploying.**
- Version 2.6.0 makes the footer notes typeable. Click the ruled area at the foot of the sheet and type; blur commits, Escape cancels, and Enter starts a new line rather than committing, since this is prose. Notes are stored against the night they belong to, in a new column existing databases gain on launch, and a new report starts empty rather than inheriting the previous night's. Because compaction already treats the notes block as its floor, a long note compacts the columns instead of colliding with them. Manual preview zoom now reaches 200% instead of stopping at 95%, which was just short of actual size — the one setting worth having when checking a 7.5pt location code — and steps coarsen above actual size so the top of the range is a dozen clicks rather than thirty; Fit no longer caps at 90%, which had left a large monitor showing a needlessly small page. Print setup gains two scoped resets: one clearing every manually dragged card width, and one restoring page margin, content scale and printer offsets to their shipped values behind a confirmation, kept apart because the latter discards calibration done for a specific printer. Also fixes a calibration checkbox that had been stretching to full width, and range sliders that rendered in the browser's default blue, both from the 2.4.0 stylesheet rewrite. **The printed sheet gained the notes area in this release; run the physical print-quality gate below before deploying.**
