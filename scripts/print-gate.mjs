...
const run = async () => {
  ...
  try {
    ...
    for (const testCase of CASES) {
      ...
      const status = problems.length ? `FAIL — ${problems.join("; ")}` : `ok  (tightened ${(report.tighten * 100).toFixed(0)}%, ${report.bottomClearanceIn}in clear, widest card ${report.widestCardIn}in)`;
